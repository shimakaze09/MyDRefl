...
int main() {
  {  // register
    ...
    field_x.value = Field::NonStaticVar{function{[](void* p) -> any {
                                          return Field::GetVar<float>(p, 0);
                                        }},
                                        function{[](void* p, any value) {
                                          Field::GetVar<float>(p, 0) =
                                              any_cast<float>(value);
                                        }}};
  ...
  // set
  get<Field::NonStaticVar>(type.fields.elems["x"].value)
      .setter(point.data(), 1.f);
  get<Field::NonStaticVar>(type.fields.elems["y"].value)
      .setter(point.data(), 2.f);
