...
int main() {
  ...
  size_t ID_operator_add_assign =
      NameRegistry::Instance().Register("operator+=");
  ...
  {  // register Vec
    ...
    Parameter param{ID_const_Vec_ptr, ID_p, true, sizeof(const Vec*),
                    alignof(const Vec*)};
  ...
  TypeInfoMngr::Instance()
      .typeinfos.at(ID_Vec)
      .methodinfos.find(ID_NormalizeSelf)
      ->second.method.Invoke(ptr);
  std::cout << v.x << ", " << v.y << std::endl;

  {
    auto rst = TypeInfoMngr::Instance()
                   .typeinfos.at(ID_Vec)
                   .cmethodinfos.find(ID_Norm2)
                   ->second.method.Invoke(ptr);
...

  {
    ...
    auto rst = TypeInfoMngr::Instance()
                   .typeinfos.at(ID_Vec)
                   .methodinfos.find(ID_operator_add_assign)
                   ->second.method.Invoke(ptr, buffer);
