...
struct Vec {
  ...
  static constexpr size_t Dim() noexcept { return 2; }
};
...
int main() {
  ...
  auto ID_const_Vec_lref =
      ReflMngr::Instance().tregistry.Register<const Vec&>();
  ...
  auto ID_NormalizeSelf =
      ReflMngr::Instance().nregistry.Register("NormalizeSelf");
  auto ID_operator_assign_add = ReflMngr::Instance().nregistry.Register(
  ...
  {  // register Vec
    ...
    MethodPtr method_Norm2 =
        ReflMngr::Instance().GenerateMethodPtr<&Vec::Norm2>();
    MethodPtr method_NormalizeSelf =
        ReflMngr::Instance().GenerateMethodPtr<&Vec::NormalizeSelf>();
    MethodPtr method_operator_add_assign =
        ReflMngr::Instance().GenerateMethodPtr<&Vec::operator+= >();
...

  {
    ...
    auto [success, typeID, destructor] = ReflMngr::Instance().Invoke(
        ptr, ID_operator_assign_add, argTypeIDs, args_buffer, result_buffer);
    assert(success);
...

  {
    ...
    std::uint8_t args_buffer[sizeof(const Vec*)];
    buffer_get<const Vec*>(args_buffer, 0) = &w;
    auto& v = ReflMngr::Instance().InvokeRet<Vec&>(ptr, ID_operator_assign_add,
