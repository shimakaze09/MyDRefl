...
int main() {
  ...
  for (const auto& [ID_field, fieldinfo] :
       TypeInfoMngr::Instance().typeinfos.at(ID_A).fieldinfos) {
  ...
  for (const auto& [ID_field, cfieldinfo] :
       TypeInfoMngr::Instance().typeinfos.at(ID_A).cfieldinfos) {
  ...
  for (const auto& [ID_field, sfieldinfo] :
       TypeInfoMngr::Instance().typeinfos.at(ID_A).sfieldinfos) {
    auto field_name = NameRegistry::Instance().Nameof(ID_field);
    if (sfieldinfo.objptr.GetID() == ID_int)
      std::cout << field_name << ": " << sfieldinfo.objptr.As<int>()
                << std::endl;
  }

  for (const auto& [ID_field, scfieldinfo] :
       TypeInfoMngr::Instance().typeinfos.at(ID_A).scfieldinfos) {
    auto field_name = NameRegistry::Instance().Nameof(ID_field);
    if (scfieldinfo.objptr.GetID() == ID_int)
      std::cout << field_name << ": " << scfieldinfo.objptr.As<int>()
                << std::endl;
  }
