...
int main() {
  ...
  for (const auto& [ID_field, fieldinfo] :
       ReflMngr::Instance().typeinfos.at(ID_A).fieldinfos) {
  ...
  for (const auto& [ID_field, cfieldinfo] :
       ReflMngr::Instance().typeinfos.at(ID_A).cfieldinfos) {
  ...
  for (const auto& [ID_field, sfieldinfo] :
       ReflMngr::Instance().typeinfos.at(ID_A).sfieldinfos) {
    auto field_name = ReflMngr::Instance().registry.Nameof(ID_field);
  ...
  for (const auto& [ID_field, scfieldinfo] :
       ReflMngr::Instance().typeinfos.at(ID_A).scfieldinfos) {
    auto field_name = ReflMngr::Instance().registry.Nameof(ID_field);
