...
int main() {
  ...
  auto ID_operator_add_assign =
      ReflMngr::Instance().nregistry.GetID("operator+=");
  ...
  {  // register Vec
    ...
    auto operator_add_assign_1 = [](void* obj, ArgsView args,
      ...
      buffer_as<Vec*>(result_buffer) = &(v += *args.At(0).As<const Vec*>());
    ...
    auto operator_add_assign_2 = [](void* obj, ArgsView args,
      ...
      auto& v = buffer_as<Vec>(obj);
      buffer_as<Vec*>(result_buffer) = &(v += args.At(0).As<float>());
      return destructor<Vec*>();
    };
...

  {
    ...
    auto [success, typeID, destructor] = ReflMngr::Instance().Invoke(
        ptr, ID_operator_add_assign, argTypeIDs, args_buffer, result_buffer);
    assert(success);
...

  {
    ...
    auto& v = ReflMngr::Instance().Invoke<Vec&, const Vec&>(
        ptr, ID_operator_add_assign, Vec{10.f, 10.f});
    std::cout << v.x << ", " << v.y << std::endl;
  }

  {
    auto& v = ReflMngr::Instance().Invoke<Vec&, float>(
        ptr, ID_operator_add_assign, 2.f);
    std::cout << v.x << ", " << v.y << std::endl;
  }
