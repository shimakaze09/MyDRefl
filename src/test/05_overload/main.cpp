...
int main() {
  ...
  {  // register Vec
    ...
    Method method_operator_add_assign_1{
    ...
    auto operator_add_assign_2 = [](void* obj, ArgsView args,
      ...
      assert(args.GetParamList().GetParameters().at(0).typeID ==
             ReflMngr::Instance().tregistry.GetID("float"));
      *reinterpret_cast<Vec**>(result_buffer) =
          &(*reinterpret_cast<Vec*>(obj) += args.At(0).As<float>());
      return destructor<Vec*>();
    };
    Method method_operator_add_assign_2{
...

  {
    ...
    *reinterpret_cast<float*>(args_buffer) = 2.f;
    auto [success, typeID, destructor] = ReflMngr::Instance().Invoke(
        ptr, ID_operator_add_assign, std::array{ID_float}, args_buffer,
        result_buffer);
    assert(success);
    assert(typeID == ID_Vec_ptr);
    assert(!destructor);
