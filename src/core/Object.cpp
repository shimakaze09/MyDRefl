...
InvokeResult ObjectView::Invoke(StrID methodID, void* result_buffer,
...
void ObjectView::ForEachOwnedVar(
    const std::function<bool(TypeRef, FieldRef, ObjectView)>& func) const {
...
ObjectView::GetTypeFieldVars() const {
...
std::vector<std::tuple<TypeRef, FieldRef, ObjectView>>
ObjectView::GetTypeFieldOwnedVars() const {
...
InvocableResult ObjectView::IsInvocable(
...
ObjectView ObjectView::tuple_get(std::size_t i) const {
  return Mngr.Invoke<ObjectView>(*this, StrIDRegistry::MetaID::tuple_get,
                                 std::move(i));
}
