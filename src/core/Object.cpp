...
Type ObjectView::BInvoke(Name method_name, void* result_buffer,
                         std::span<const Type> argTypes,
                         ArgPtrBuffer argptr_buffer, MethodFlag flag) const {
  return Mngr->BInvoke(*this, method_name, result_buffer, argTypes,
                       argptr_buffer, flag);
}
...
SharedObject ObjectView::MInvoke(Name method_name,
  ...
  return Mngr->MInvoke(*this, method_name, rst_rsrc, temp_args_rsrc, argTypes,
                       argptr_buffer, flag);
}

SharedObject ObjectView::Invoke(Name method_name,
                                std::span<const Type> argTypes,
                                ArgPtrBuffer argptr_buffer,
                                MethodFlag flag) const {
...
std::vector<InfoTypeMethodPair> ObjectView::GetTypeMethods(
    MethodFlag flag) const {
...
InfoTypePair ObjectView::FindType(
    const std::function<bool(InfoTypePair)>& func) const {
  return Mngr->FindType(type, func);
}

InfoFieldPair ObjectView::FindField(
    const std::function<bool(InfoFieldPair)>& func, FieldFlag flag) const {
  return Mngr->FindField(type, func, flag);
}

InfoMethodPair ObjectView::FindMethod(
    const std::function<bool(InfoMethodPair)>& func, MethodFlag flag) const {
...
ObjectView ObjectView::tuple_get(std::size_t i) const {
  return Mngr->BInvoke<ObjectView>(*this, NameIDRegistry::Meta::tuple_get,
                                   std::move(i));
}
...
std::size_t ObjectView::size() const {
  return Mngr->BInvoke<std::size_t>(*this,
...
std::size_t ObjectView::bucket_count() const {
  return Mngr->BInvoke<std::size_t>(
      *this, NameIDRegistry::Meta::container_bucket_count);
}
