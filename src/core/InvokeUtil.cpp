...
bool details::RefConstruct(ObjectView obj, std::span<const Type> argTypes,
  ...
  for (auto iter = begin_iter; iter != end_iter; ++iter) {
    if (iter->second.methodptr.GetMethodFlag() == MethodFlag::Variable &&
      ...
      iter->second.methodptr.Invoke(
          obj.GetPtr(), nullptr,
          {argptr_buffer, iter->second.methodptr.GetParamList()});
      return true;
    }
  }
  return false;
...
details::NewArgsGuard::NewArgsGuard(bool is_priority,
  ...
  if (contains_objview) {
    new (&type_buffer)
        BufferGuard{rsrc, num_args * sizeof(Type), alignof(Type)};
  ...
  for (std::uint8_t k = 0; k < num_copiedargs; ++k) {
    ...
    std::uint32_t offset =
        (size_copiedargs + (alignment - 1)) & ~(alignment - 1);
  ...
  std::uint32_t offset_new_argptr_buffer =
      (size_copiedargs + alignof(void*) - 1) & ~(alignof(void*) - 1);
  std::uint32_t offset_new_nonptr_arg_info_buffer =
      offset_new_argptr_buffer + num_args * sizeof(void*);

  std::uint32_t buffer_size = offset_new_nonptr_arg_info_buffer +
  ...
  auto new_nonptr_arg_info_buffer = reinterpret_cast<ArgInfo*>(
  ...
  for (std::uint8_t i = 0; i < num_args; i++) {
    ...
    if (info.is_pointer_or_array)
      ...
      bool success =
          RefConstruct(ObjectView{info.GetType(), arg_buffer},
