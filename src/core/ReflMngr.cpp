    ...
    for (std::uint16_t i = 0; i < num_args; i++) {
      ...
      if (info_copiedargs[idx_copiedargs].is_ptr)
        ...
        bool success = Mngr.NonArgConstruct(
            ObjectView{TypeID{info_copiedargs[idx_copiedargs].typeID},
                       arg_buffer},
...
static InvokeResult Invoke(bool is_priority,
  ...
  for (const auto& [baseID, baseinfo] : typeinfo.baseinfos) {
    auto rst = Invoke(
        ...
        ObjectView{baseID, baseinfo.StaticCast_DerivedToBase(obj.GetPtr())},
...
static SharedObject MInvoke(bool is_priority,
                            ...
                            ObjectView obj, bool is_const, StrID methodID,
                            std::span<const TypeID> argTypeIDs,
                            ArgPtrBuffer argptr_buffer,
                            std::pmr::memory_resource* rst_rsrc) {
  assert(rst_rsrc);
...
ObjectView ReflMngr::NonArgCopyNew(TypeID typeID,
                                   std::span<const TypeID> argTypeIDs,
...
ObjectView ReflMngr::New(TypeID typeID, std::span<const TypeID> argTypeIDs,
                         ArgPtrBuffer argptr_buffer) const {
...
ObjectView ReflMngr::StaticCast_DerivedToBase(ObjectView obj,
                                              TypeID typeID) const {
  assert(typeID);
...
ObjectView ReflMngr::StaticCast_BaseToDerived(ObjectView obj,
                                              TypeID typeID) const {
  assert(typeID);
...
ObjectView ReflMngr::DynamicCast_BaseToDerived(ObjectView obj,
  ...
  for (const auto& [baseID, baseinfo] : typeinfo.baseinfos) {
    auto ptr = DynamicCast_BaseToDerived(
        ObjectView{baseID, baseinfo.DynamicCast_BaseToDerived(obj.GetPtr())},
        typeID);
    if (ptr.GetTypeID())
...
ObjectView ReflMngr::Var(ObjectView obj, StrID fieldID) {
  ...
  for (const auto& [baseID, baseinfo] : typeinfo.baseinfos) {
    auto bptr =
        Var(ObjectView{baseID, baseinfo.StaticCast_DerivedToBase(obj.GetPtr())},
            fieldID);
    if (bptr.GetTypeID())
      return bptr;
  }
...
InvokeResult ReflMngr::Invoke(ObjectView obj, StrID methodID,
                              void* result_buffer,
                              std::span<const TypeID> argTypeIDs,
                              ArgPtrBuffer argptr_buffer) const {
  ObjectView rawObj;
...
ObjectView ReflMngr::NonArgCopyMNew(TypeID typeID,
                                    std::pmr::memory_resource* rsrc,
                                    std::span<const TypeID> argTypeIDs,
                                    ArgPtrBuffer argptr_buffer) const {
  assert(rsrc);
...
bool ReflMngr::NonArgConstruct(ObjectView obj,
                               std::span<const TypeID> argTypeIDs,
                               ArgPtrBuffer argptr_buffer) const {
...
bool ReflMngr::Construct(ObjectView obj, std::span<const TypeID> argTypeIDs,
                         ArgPtrBuffer argptr_buffer) const {
  assert(GetConstReferenceMode(obj.GetTypeID()) == ConstReferenceMode::None);
...
ObjectView ReflMngr::FindVar(
    TypeID typeID, const std::function<bool(ObjectView)>& func) const {
  ObjectView rst;
  ForEachVar(typeID,
