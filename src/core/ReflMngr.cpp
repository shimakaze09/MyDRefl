...
ObjectPtr ReflMngr::Cast(ObjectPtr obj, size_t typeID) const noexcept {
  ...
  {  // as base
    ...
    if (typetarget != typeinfos.end()) {
      ...
      if (basetarget != typetarget->second.baseinfos.end()) {
        return {typeID, backward_offset(obj, basetarget->second.offset)};
      }
    }
  }
  ...
  {  // as derived
    ...
    if (typetarget != typeinfos.end()) {
      ...
      if (basetarget != typetarget->second.baseinfos.end()) {
        return {typeID, forward_offset(obj, basetarget->second.offset)};
      }
    }
  }
...
ObjectPtr ReflMngr::RWField(ObjectPtr obj, size_t fieldID) const noexcept {
  ...
  for (const auto& [baseID, baseinfo] : typeinfo.baseinfos) {
    auto bptr = RWField(ObjectPtr{baseID, forward_offset(obj, baseinfo.offset)},
                        fieldID);
    if (bptr)
      return bptr;
  }
...
ConstObjectPtr ReflMngr::RField(ConstObjectPtr obj,
  ...
  for (const auto& [baseID, baseinfo] : typeinfo.baseinfos) {
    auto bptr = RField(
        ConstObjectPtr{baseID, forward_offset(obj, baseinfo.offset)}, fieldID);
    if (bptr)
      return bptr;
  }
...
InvokeResult ReflMngr::Invoke(ConstObjectPtr obj, size_t methodID,
  ...
  for (const auto& [baseID, baseinfo] : typeinfo.baseinfos) {
    auto rst =
        Invoke(ConstObjectPtr{baseID, forward_offset(obj, baseinfo.offset)},
...
InvokeResult ReflMngr::Invoke(ObjectPtr obj, size_t methodID,
  ...
  for (const auto& [baseID, baseinfo] : typeinfo.baseinfos) {
    auto rst = Invoke(ObjectPtr{baseID, forward_offset(obj, baseinfo.offset)},
                      methodID, argTypeIDs, buffer);
    if (rst.success)
      return rst;
  }
