...
void AddConvertCtor(ReflMngr& mngr) {
  ...
  if constexpr (!std::is_same_v<To, From>) {
    mngr.AddMemberMethod(NameIDRegistry::Meta::ctor,
 ...
 public:
  NewArgsGuard(bool is_priority, std::pmr::memory_resource* rsrc,
    ...
    for (std::uint8_t i = 0; i < argTypes.size(); i++) {
      ...
      if (lhs.IsLValueReference()) {  // &{T} | &{const{T}}
        const auto unref_lhs =
            lhs.Name_RemoveLValueReference();  // T | const{T}
        if (type_name_is_const(unref_lhs)) {   // &{const{T}}
          if (unref_lhs == rhs.Name_RemoveRValueReference())
            continue;  // &{const{T}} <- &&{const{T}}
          ...
          if (Mngr.IsNonCopiedArgConstructible(
                  raw_lhs_type, std::span<const Type>{&rhs, 1})) {
        ...
        const auto unref_lhs =
            lhs.Name_RemoveRValueReference();  // T | const{T}
        if (type_name_is_const(unref_lhs)) {   // &&{const{T}}
          const auto raw_lhs = type_name_remove_const(unref_lhs);  // T
    ...
    info_copiedargs[num_copiedargs].idx =
        static_cast<std::uint8_t>(-1);  // guard
...
static InvocableResult IsStaticInvocable(bool is_priority, Type type,
                                         Name method_name,
                                         std::span<const Type> argTypes) {
...
static InvocableResult IsConstInvocable(bool is_priority, Type type,
                                        Name method_name,
                                        std::span<const Type> argTypes) {
  assert(type.CVRefMode() == CVRefMode::None);
  auto typetarget = Mngr.typeinfos.find(type);
...
static InvocableResult IsInvocable(bool is_priority, Type type,
                                   Name method_name,
                                   std::span<const Type> argTypes) {
  assert(type.CVRefMode() == CVRefMode::None);
  auto typetarget = Mngr.typeinfos.find(type);
...
static InvokeResult Invoke(bool is_priority,
  ...
  for (auto iter = begin_iter; iter != end_iter; ++iter) {
    if (iter->second.methodptr.IsStatic()) {
      ...
      return {true, iter->second.methodptr.GetResultDesc().type,
  ...
  for (const auto& [base, baseinfo] : typeinfo.baseinfos) {
    auto rst = Invoke(is_priority, args_rsrc, base, method_name, result_buffer,
...
static InvokeResult Invoke(bool is_priority,
                           std::pmr::memory_resource* args_rsrc, ObjectView obj,
                           bool is_const, Name method_name, void* result_buffer,
                           std::span<const Type> argTypes,
                           ArgPtrBuffer argptr_buffer) {
  ...
  for (const auto& [base, baseinfo] : typeinfo.baseinfos) {
    auto rst = Invoke(
        ...
        is_const, method_name, result_buffer, argTypes, argptr_buffer);
    if (rst.success)
      return rst;
  }
    ...
    if (iter->second.methodptr.IsStatic()) {
      ...
      if (rst_desc.type.Is<void>()) {
        ...
        auto dtor = iter->second.methodptr.Invoke(result_buffer,
                                                  guard.GetArgPtrBuffer());
        return {{rst_desc.type, result_buffer},
...
static bool ForEachTypeInfo(Type type, const std::function<bool(TypeRef)>& func,
...
static bool ForEachVar(
    Type type, const std::function<bool(TypeRef, FieldRef, ObjectView)>& func,
    std::set<TypeID>& visitedVBs) {
  auto target = Mngr.typeinfos.find(type);
...
#define MY_MYDREFL_ALIGN_FREE free
  ...
  AddMethod<&MY_MYDREFL_ALIGN_MALLOC>(Global,
                                      NameIDRegistry::Meta::aligned_malloc);
  AddMethod<&free>(Global, NameIDRegistry::Meta::free);
  AddMethod<&MY_MYDREFL_ALIGN_FREE>(Global, NameIDRegistry::Meta::aligned_free);
...
bool ReflMngr::Free(void* ptr) const {
  return InvokeArgs(Global, NameIDRegistry::Meta::free, nullptr,
                    std::move(ptr));
}

void* ReflMngr::AlignedMalloc(size_t size, size_t alignment) const {
  return Invoke<void*>(Global, NameIDRegistry::Meta::aligned_malloc,
                       std::move(size), std::move(alignment));
}

bool ReflMngr::AlignedFree(void* ptr) const {
  return InvokeArgs(Global, NameIDRegistry::Meta::aligned_free, nullptr,
                    std::move(ptr));
}

ObjectView ReflMngr::NonArgCopyNew(Type type, std::span<const Type> argTypes,
...
ObjectView ReflMngr::New(Type type, std::span<const Type> argTypes,
                         ArgPtrBuffer argptr_buffer) const {
...
SharedObject ReflMngr::MakeShared(Type type, std::span<const Type> argTypes,
                                  ArgPtrBuffer argptr_buffer) const {
...
ObjectView ReflMngr::StaticCast_DerivedToBase(ObjectView obj, Type type) const {
  ...
  switch (cvref_mode) {
    case CVRefMode::Left:
      return StaticCast_DerivedToBase(obj.RemoveReference(), type)
          .AddLValueReference();
    ...
    case CVRefMode::ConstLeft:
      return StaticCast_DerivedToBase(obj.RemoveConstReference(), type)
...
ObjectView ReflMngr::StaticCast_BaseToDerived(ObjectView obj, Type type) const {
  ...
  switch (cvref_mode) {
    case CVRefMode::Left:
      return StaticCast_BaseToDerived(obj.RemoveReference(), type)
          .AddLValueReference();
    case CVRefMode::Right:
      return StaticCast_BaseToDerived(obj.RemoveReference(), type)
          .AddRValueReference();
    ...
    case CVRefMode::ConstLeft:
      return StaticCast_BaseToDerived(obj.RemoveConstReference(), type)
          .AddConstLValueReference();
    case CVRefMode::ConstRight:
      return StaticCast_BaseToDerived(obj.RemoveConstReference(), type)
          .AddConstRValueReference();
...
ObjectView ReflMngr::DynamicCast_BaseToDerived(ObjectView obj,
  ...
  switch (cvref_mode) {
    case CVRefMode::Left:
      return DynamicCast_BaseToDerived(obj.RemoveReference(), type)
          .AddLValueReference();
    case CVRefMode::Right:
      return DynamicCast_BaseToDerived(obj.RemoveReference(), type)
          .AddRValueReference();
    ...
    case CVRefMode::ConstLeft:
      return DynamicCast_BaseToDerived(obj.RemoveConstReference(), type)
          .AddConstLValueReference();
    case CVRefMode::ConstRight:
      return DynamicCast_BaseToDerived(obj.RemoveConstReference(), type)
          .AddConstRValueReference();
  ...
  for (size_t i = 0; i < params.size(); i++) {
    ...
    if (lhs.IsLValueReference()) {  // &{T} | &{const{T}}
      ...
      if (type_name_is_const(unref_lhs)) {                      // &{const{T}}
        ...
        if (IsNonCopiedArgConstructible(raw_lhs,
                                        std::span<const Type>{&rhs, 1}))
          continue;  // &{const{T}} <- T{arg}
      }
    } else if (lhs.IsRValueReference()) {  // &&{T} | &&{const{T}}
      ...
      if (type_name_is_const(unref_lhs)) {                       // &&{const{T}}
        ...
        if (IsNonCopiedArgConstructible(unref_lhs,
                                        std::span<const Type>{&rhs, 1}))
          continue;  // &&{T} <- T{arg}
      }
    } else {  // T
...
InvocableResult ReflMngr::IsStaticInvocable(
    Type type, Name method_name, std::span<const Type> argTypes) const {
  const CVRefMode cvref_mode = type.CVRefMode();
  assert(!CVRefMode_IsVolatile(cvref_mode));
  switch (cvref_mode) {
    case CVRefMode::Left:
  ...
  if (auto priority_rst =
          details::IsStaticInvocable(true, type, method_name, argTypes))
    return priority_rst;
...
InvocableResult ReflMngr::IsConstInvocable(
  ...
  if (auto priority_rst =
          details::IsConstInvocable(true, type, method_name, argTypes))
    return priority_rst;
...
InvocableResult ReflMngr::IsInvocable(Type type, Name method_name,
  ...
  if (auto priority_rst =
          details::IsInvocable(true, type, method_name, argTypes))
    return priority_rst;

  return details::IsInvocable(false, type, method_name, argTypes);
}

InvokeResult ReflMngr::Invoke(Type type, Name method_name, void* result_buffer,
                              std::span<const Type> argTypes,
                              ArgPtrBuffer argptr_buffer) const {
  Type rawType;
  const CVRefMode cvref_mode = type.CVRefMode();
  ...
  if (auto priority_rst =
          details::Invoke(true, &temporary_resource, rawType, method_name,
                          result_buffer, argTypes, argptr_buffer))
    return priority_rst;

  return details::Invoke(false, &temporary_resource, rawType, method_name,
                         result_buffer, argTypes, argptr_buffer);
}

InvokeResult ReflMngr::Invoke(ObjectView obj, Name method_name,
                              void* result_buffer,
                              std::span<const Type> argTypes,
                              ArgPtrBuffer argptr_buffer) const {
  ObjectView rawObj;
  ...
  if (auto priority_rst =
          details::Invoke(true, &temporary_resource, rawObj, is_const,
                          method_name, result_buffer, argTypes, argptr_buffer))
    return priority_rst;

  return details::Invoke(false, &temporary_resource, rawObj, is_const,
                         method_name, result_buffer, argTypes, argptr_buffer);
}
...
bool ReflMngr::IsNonCopiedArgConstructible(
  ...
  for (size_t i = 0; i < num; ++i, ++mtarget) {
    if (details::IsNonCopiedArgConstructCompatible(
            mtarget->second.methodptr.GetParamList(), argTypes))
      return true;
  }
  return false;
}
...
void ReflMngr::ForEachField(
    Type type, const std::function<bool(TypeRef, FieldRef)>& func) const {
...
void ReflMngr::ForEachMethod(
    Type type, const std::function<bool(TypeRef, MethodRef)>& func) const {
  ForEachTypeInfo(type, [&func](TypeRef type) {
...
void ReflMngr::ForEachVar(
  ...
  switch (cvref_mode) {
    ...
    case CVRefMode::Right:
      details::ForEachVar(
          obj.RemoveReference(),
          [&func](TypeRef t, FieldRef f, ObjectView o) {
...
std::vector<std::tuple<TypeRef, FieldRef, ObjectView>>
ReflMngr::GetTypeFieldVars(Type type) {
...
std::vector<std::tuple<TypeRef, FieldRef, ObjectView>>
ReflMngr::GetTypeFieldVars(ObjectView obj) {
  std::vector<std::tuple<TypeRef, FieldRef, ObjectView>> rst;
...
std::vector<std::tuple<TypeRef, FieldRef, ObjectView>>
ReflMngr::GetTypeFieldOwnedVars(ObjectView obj) {
  std::vector<std::tuple<TypeRef, FieldRef, ObjectView>> rst;
  ForEachVar(obj, [&rst](TypeRef type, FieldRef field, ObjectView var) {
...
ObjectView ReflMngr::FindVar(
    Type type, const std::function<bool(ObjectView)>& func) const {
  ObjectView rst;
...
ObjectView ReflMngr::FindVar(
    ObjectView obj, const std::function<bool(ObjectView)>& func) const {
  ObjectView rst;
...
ObjectView ReflMngr::FindOwnedVar(
    ObjectView obj, const std::function<bool(ObjectView)>& func) const {
  ObjectView rst;
  ForEachVar(obj, [&rst, func](TypeRef type, FieldRef field, ObjectView obj) {
