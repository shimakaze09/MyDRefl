...
class FieldPtr {
 public:
  ...
  constexpr FieldPtr(TypeID valueID, size_t forward_offset_value,
                     std::true_type is_const) noexcept
      : valueID{valueID},
  ...
  constexpr FieldPtr(TypeID valueID, size_t forward_offset_value,
                     std::false_type is_const) noexcept
      : valueID{valueID},
  ...
  constexpr FieldPtr(ObjectPtr static_obj) noexcept
      : FieldPtr{static_obj.GetID(), static_obj.GetPtr()} {}

  constexpr FieldPtr(ConstObjectPtr static_obj) noexcept
      : FieldPtr{static_obj.GetID(), static_obj.GetPtr()} {}
  ...
  ConstObjectPtr Map() const noexcept {
    return std::visit(
        [this](const auto& offset) -> ConstObjectPtr {
  ...
  ConstObjectPtr Map(const void* obj) const noexcept {
    return std::visit(
        [this, obj](const auto& offset) -> ConstObjectPtr {
          using T = std::decay_t<decltype(offset)>;
          if constexpr (std::is_same_v<T, size_t>) {
            assert(obj);
