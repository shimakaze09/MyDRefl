...
class SharedBuffer {
 public:
  ...
  SharedBuffer(void* ptr, std::function<void(void*)> deleter)
      : ptr{ptr, deleter} {}
  ...
  SharedBuffer(std::nullptr_t ptr, Deleter&& d)
      : ptr{ptr, std::forward<Deleter>(d)} {}

  template <typename U, typename Deleter, typename Alloc>
  SharedBuffer(U* ptr, Deleter&& d, Alloc&& alloc)
      : ptr{ptr, std::forward<Deleter>(d), std::forward<Alloc>(alloc)} {}

  template <typename Deleter, typename Alloc>
  SharedBuffer(std::nullptr_t ptr, Deleter&& d, Alloc&& alloc)
      : ptr{ptr, std::forward<Deleter>(d), std::forward<Alloc>(alloc)} {}
  ...
  std::shared_ptr<const void> CastToSharedPtr() const& noexcept {
  ...
  operator std::shared_ptr<const void>() const& noexcept {
    return std::static_pointer_cast<const void>(ptr);
  }
  ...
  const T* AsPtr() const noexcept {
    return const_cast<SharedBuffer*>(this)->AsPtr<T>();
  }
...
template <>
struct std::hash<My::MyDRefl::SharedBuffer> {
  std::size_t operator()(const My::MyDRefl::SharedBuffer& obj) const noexcept {
    return std::hash<const void*>()(obj.Get());
  }
};

inline bool operator==(const My::MyDRefl::SharedBuffer& left,
                       const My::MyDRefl::SharedBuffer& right) noexcept {
...
std::basic_ostream<Elem, Traits>& operator<<(
    ...
    const My::MyDRefl::SharedBuffer& obj) {
  return out << obj.Get();
}

namespace std {
inline void swap(My::MyDRefl::SharedBuffer& left,
                 My::MyDRefl::SharedBuffer& right) noexcept {
  left.Swap(right);
}
