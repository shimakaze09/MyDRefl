...
#define OBJECT_VIEW_DEFINE_ASSIGN_OP_OPERATOR(op, name)                     \
  ...
  ObjectView ObjectView::operator op(T&& rhs) const {                       \
    Invoke<void>(NameIDRegistry::Meta::operator_##name,                     \
...
#define OBJECT_VIEW_DEFINE_META_T(fname, mname)        \
  ...
  SharedObject ObjectView::fname(T&& arg) const {      \
    return Invoke(NameIDRegistry::Meta::mname,         \
                  TempArgsView{std::forward<T>(arg)}); \
  }

#define OBJECT_VIEW_DEFINE_OPERATOR_T(op, name) \
  OBJECT_VIEW_DEFINE_META_T(operator op, operator_##name)
...
#define OBJECT_VIEW_DEFINE_META_VARS_T(prefix, name)          \
  ...
  SharedObject ObjectView::name(Args&&... args) const {       \
    return Invoke(NameIDRegistry::Meta::prefix##_##name,      \
...
template <typename T>
T ObjectView::Invoke(Name method_name, ArgsView args, MethodFlag flag,
  ...
  if constexpr (!std::is_void_v<T>) {
    ...
    Type result_type =
        BInvoke(method_name, &result_buffer, args, flag, temp_args_rsrc);
...
void ObjectView::assign(Args&&... args) const {
  Invoke<void>(NameIDRegistry::Meta::container_assign,
...
inline void ObjectView::clear() const {
  Invoke<void>(NameIDRegistry::Meta::container_clear, ArgsView{},
               MethodFlag::Variable);
}
...
std::size_t ObjectView::count(T&& arg) const {
  return Invoke<std::size_t>(NameIDRegistry::Meta::container_count,
...
inline bool operator==(const ObjectView& lhs, const ObjectView& rhs) {
  return lhs.Invoke<bool>(NameIDRegistry::Meta::operator_eq, TempArgsView{rhs},
                          MethodFlag::Const) ||
         rhs.Invoke<bool>(NameIDRegistry::Meta::operator_eq, TempArgsView{lhs},
                          MethodFlag::Const);
}
...
template <NonObjectAndView T>
bool operator==(ObjectView ptr, const T& rhs) {
  return ptr == ObjectView{rhs};
}
