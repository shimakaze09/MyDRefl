...
#define OBJECT_VIEW_DEFINE_OPERATOR_T(op, name)           \
  ...
  SharedObject ObjectView::operator op(T&& rhs) const {   \
    return AInvoke(NameIDRegistry::Meta::operator_##name, \
                   std::forward<T>(rhs));                 \
  }
...
template <typename T, typename... Args>
T ObjectView::ABInvoke(Name method_name, MethodFlag flag,
                       Args&&... args) const {
  if constexpr (sizeof...(Args) > 0) {
    constexpr Type argTypes[] = {Type_of<decltype(args)>...};
    void* const argptr_buffer[] = {
...
bool ObjectView::operator<(const T& rhs) const {
  return ABInvoke<bool>(NameIDRegistry::Meta::operator_lt, MethodFlag::Const,
                        rhs);
}

template <typename T>
bool ObjectView::operator<=(const T& rhs) const {
  return ABInvoke<bool>(NameIDRegistry::Meta::operator_le, MethodFlag::Const,
                        rhs);
}

template <typename T>
bool ObjectView::operator>(const T& rhs) const {
  return ABInvoke<bool>(NameIDRegistry::Meta::operator_gt, MethodFlag::Const,
                        rhs);
}

template <typename T>
bool ObjectView::operator>=(const T& rhs) const {
  return ABInvoke<bool>(NameIDRegistry::Meta::operator_ge, MethodFlag::Const,
                        rhs);
}
...
void ObjectView::assign(Args&&... args) const {
  ABInvoke<void>(NameIDRegistry::Meta::container_assign, MethodFlag::Variable,
                 std::forward<Args>(args)...);
};
...
void ObjectView::resize(Args&&... args) const {
  ABInvoke<void>(NameIDRegistry::Meta::container_resize, MethodFlag::Variable,
                 std::forward<Args>(args)...);
};

inline void ObjectView::reserve(std::size_t n) const {
  BInvoke<void>(NameIDRegistry::Meta::container_reserve, MethodFlag::Variable,
                std::move(n));
}
...
void ObjectView::push(T&& arg) const {
  ABInvoke<void>(NameIDRegistry::Meta::container_push, MethodFlag::Variable,
                 std::forward<T>(arg));
};

inline void ObjectView::pop() const {
  BInvoke<void>(NameIDRegistry::Meta::container_pop, MethodFlag::Variable);
}

template <typename T>
void ObjectView::swap(T&& arg) const {
  ABInvoke<void>(NameIDRegistry::Meta::container_swap, MethodFlag::Variable,
                 std::forward<T>(arg));
};
...
void ObjectView::splice_after(Args&&... args) const {
  ABInvoke<void>(NameIDRegistry::Meta::container_splice_after,
                 MethodFlag::Variable, std::forward<Args>(args)...);
};

template <typename... Args>
void ObjectView::splice(Args&&... args) const {
  ABInvoke<void>(NameIDRegistry::Meta::container_splice, MethodFlag::Variable,
                 std::forward<Args>(args)...);
};
...
inline void ObjectView::reverse() const {
  BInvoke<void>(NameIDRegistry::Meta::container_reverse, MethodFlag::Variable);
}
...
inline bool operator==(const ObjectView& lhs, const ObjectView& rhs) {
  return static_cast<bool>(
             lhs.AInvoke(NameIDRegistry::Meta::operator_eq, rhs)) ||
         static_cast<bool>(rhs.AInvoke(NameIDRegistry::Meta::operator_eq, lhs));
}

inline bool operator!=(const ObjectView& lhs, const ObjectView& rhs) {
  return static_cast<bool>(
             lhs.AInvoke(NameIDRegistry::Meta::operator_ne, rhs)) ||
