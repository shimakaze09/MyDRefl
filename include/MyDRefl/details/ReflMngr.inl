...
struct TypeAutoRegister_Default {
  static void run(ReflMngr& mngr) {
    ...
    if constexpr (std::input_iterator<T>) {
      ...
      if constexpr (std::is_convertible_v<std::iter_difference_t<T>,
        ...
        mngr.AddMemberMethod(
            ...
            [](const T& lhs, const T& rhs) -> std::size_t {
    ...
    if constexpr (container_subscript_size<T>)
      mngr.AddMemberMethod(
          NameIDRegistry::Meta::operator_subscript,
          [](T& lhs, const typename T::size_type& rhs) -> decltype(auto) {
            return lhs[rhs];
          });
    if constexpr (container_subscript_size<const T>)
      mngr.AddMemberMethod(
          NameIDRegistry::Meta::operator_subscript,
          [](const T& lhs, const typename T::size_type& rhs) -> decltype(auto) {
            return lhs[rhs];
          });
    ...
    if constexpr (container_empty<T>)
      mngr.AddMemberMethod(NameIDRegistry::Meta::container_empty,
                           [](const T& lhs) -> bool {
                             return static_cast<bool>(std::empty(lhs));
                           });
    ...
    if constexpr (container_insert_citer_size_value<T>)
      mngr.AddMemberMethod(
          NameIDRegistry::Meta::container_insert,
          [](T& lhs, const typename T::const_iterator& iter,
             const typename T::size_type& size,
             const typename T::value_type& value) -> decltype(auto) {
            return lhs.insert(iter, size, value);
          });
    ...
    if constexpr (container_count<T>)
      mngr.AddMemberMethod(
          ...
          [](const T& lhs, const typename T::key_type& rhs) -> std::size_t {
    ...
    if constexpr (std::is_array_v<T>) {
      ...
      if constexpr (container_reverse_iterator<T> &&
                    container_const_reverse_iterator<T>)
        mngr.AddConstructor<typename T::const_reverse_iterator,
                            const typename T::reverse_iterator&>();
      if constexpr (container_local_iterator<T> &&
