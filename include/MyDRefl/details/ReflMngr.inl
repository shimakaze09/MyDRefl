...
SharedObject ReflMngr::MInvoke(TypeID typeID, StrID methodID,
  ...
  if constexpr (sizeof...(Args) > 0) {
    ...
    auto args_buffer =
        type_buffer_decay_as_tuple<Args...>(std::forward<Args>(args)...);
    return MInvoke(typeID, methodID, Span<TypeID>{argTypeIDs},
                   static_cast<void*>(&args_buffer), memory_rsrc_type);
  } else
    return MInvoke(typeID, methodID, Span<TypeID>{},
                   static_cast<void*>(nullptr), memory_rsrc_type);
}
...
SharedObject ReflMngr::MInvoke(ConstObjectPtr obj, StrID methodID,
  ...
  if constexpr (sizeof...(Args) > 0) {
    ...
    auto args_buffer =
        type_buffer_decay_as_tuple<Args...>(std::forward<Args>(args)...);
    return MInvoke(obj, methodID, Span<TypeID>{argTypeIDs},
                   static_cast<void*>(&args_buffer), memory_rsrc_type);
  } else
    return MInvoke(obj, methodID, Span<TypeID>{}, static_cast<void*>(nullptr),
                   memory_rsrc_type);
}
