...
struct TypeAutoRegister_Default {
  static void run(ReflMngr& mngr) {
    ...
    if constexpr (is_valid_v<operator_assign_move, T>)
      mngr.AddMemberMethod(NameIDRegistry::Meta::operator_assign,
    ...
    if constexpr (is_iterator_v<T>) {
      if constexpr (is_valid_v<operator_add, T, std::size_t>) {
        mngr.AddMemberMethod(
            ...
            [](const T& lhs,
               const typename std::iterator_traits<T>::difference_type& rhs)
      ...
      if constexpr (is_valid_v<iterator_next, T>) {
        ...
        mngr.AddMemberMethod(NameIDRegistry::Meta::iterator_next,
      ...
      if constexpr (is_valid_v<iterator_prev, T>) {
        mngr.AddMemberMethod(
            ...
            [](const T& lhs,
               const typename std::iterator_traits<T>::difference_type& rhs) {
              return std::prev(lhs, rhs);
            });
        mngr.AddMemberMethod(NameIDRegistry::Meta::iterator_prev,
    ...
    if constexpr (is_valid_v<container_empty, T>)
      mngr.AddMemberMethod(
          ...
          [](const T& lhs) -> decltype(auto) { return std::empty(lhs); });

    if constexpr (is_valid_v<container_size, T>)
      mngr.AddMemberMethod(
          NameIDRegistry::Meta::container_size,
          [](const T& lhs) -> decltype(auto) { return std::size(lhs); });
    ...
    if constexpr (is_valid_v<container_resize_0, T>)
      mngr.AddMemberMethod(NameIDRegistry::Meta::container_resize,
                           [](T& lhs, const std::size_t& n) -> decltype(auto) {
                             return lhs.resize(n);
                           });
    ...
    if constexpr (is_valid_v<container_shrink_to_fit, T>)
      mngr.AddMemberMethod(
          NameIDRegistry::Meta::container_shrink_to_fit,
    ...
    if constexpr (is_valid_v<container_clear, T>)
      mngr.AddMemberMethod(
          NameIDRegistry::Meta::container_clear,
    ...
    if constexpr (is_valid_v<container_push_front_0, T>)
      mngr.AddMemberMethod(
          NameIDRegistry::Meta::container_push_front,
    ...
    if constexpr (is_valid_v<container_count, T>)
      mngr.AddMemberMethod(
          NameIDRegistry::Meta::container_count,
    ...
    if constexpr (IsVector_v<T>)
      ...
      mngr.AddAttr(Type_of<T>,
                   mngr.MakeShared(Type_of<ContainerType>, ContainerType::Map));
    else if constexpr (IsSet_v<T>)
      mngr.AddAttr(Type_of<T>,
                   mngr.MakeShared(Type_of<ContainerType>, ContainerType::Set));
