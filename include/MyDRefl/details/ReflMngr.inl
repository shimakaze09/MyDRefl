  ...
  static void run(ReflMngr& mngr) {
    ...
    if constexpr (IsContainer_v<T>) {
      ...
      if constexpr (is_valid_v<container_find_const, T>)
        mngr.AddMemberMethod(NameIDRegistry::Meta::container_find,
                             [](const T& lhs, const typename T::key_type& rhs)
...
FieldPtr ReflMngr::GenerateDynamicFieldPtrByAlloc(const Alloc& alloc,
  ...
  if constexpr (FieldPtr::IsBufferable<RawT>()) {
    ...
    SharedObject obj = {Type_of<T>, std::allocate_shared<RawT>(
...
T ReflMngr::InvokeRet(Type type, Name method_name,
  ...
  if constexpr (!std::is_void_v<T>) {
    ...
    InvokeResult result =
        Invoke(type, method_name, result_buffer, argTypes, argptr_buffer);
...
T ReflMngr::InvokeRet(ObjectView obj, Name method_name,
  ...
  if constexpr (!std::is_void_v<T>) {
    ...
    std::uint8_t result_buffer[sizeof(U)];
    InvokeResult result =
        Invoke(obj, method_name, result_buffer, argTypes, argptr_buffer);
    assert(result.type.Is<T>());
    return result.Move<T>(result_buffer);
  } else
