...
struct TypeAutoRegister {
  static void run(ReflMngr& mngr) {
    ...
    if constexpr (is_valid_v<container_end, T>)
      mngr.AddMemberMethod(StrIDRegistry::Meta::container_end,
    ...
    if constexpr (is_valid_v<container_rend, T>)
      mngr.AddMemberMethod(StrIDRegistry::Meta::container_rend,
    ...
    if constexpr (is_valid_v<container_data, T>)
      mngr.AddMemberMethod(StrIDRegistry::Meta::container_data,
    ...
    if constexpr (is_valid_v<container_back, T>)
      mngr.AddMemberMethod(StrIDRegistry::Meta::container_back,
    ...
    if constexpr (is_valid_v<container_reserve, T>)
      mngr.AddMemberMethod(StrIDRegistry::Meta::container_capacity,
                           [](T& lhs, std::size_t n) -> decltype(auto) {
    ...
    if constexpr (is_valid_v<container_swap, T>)
      mngr.AddMemberMethod(
          StrIDRegistry::Meta::container_swap,
          [](T& lhs, T& rhs) -> decltype(auto) { return std::swap(lhs, rhs); });
    ...
    if constexpr (is_valid_v<container_count, T>)
      mngr.AddMemberMethod(
          StrIDRegistry::Meta::container_count,
...
FieldPtr ReflMngr::GenerateFieldPtr() {
  ...
  if constexpr (std::is_pointer_v<FieldData>) {
    ...
    return {TypeID::of<std::remove_const_t<Value>>, field_data};
    ...
    if constexpr (has_virtual_base_v<Object>) {
      return {TypeID::of<std::remove_const_t<Value>>,
    ...
    return {TypeID::of<std::remove_const_t<Value>>, buffer};
  } else
    static_assert(false);
}
...
FieldPtr ReflMngr::GenerateFieldPtr(T&& data) {
  ...
  if constexpr (std::is_member_object_pointer_v<RawT>) {
    ...
    RegisterTypeAuto<std::remove_const_t<Value>>();
    if constexpr (has_virtual_base_v<Object>) {
      return {TypeID::of<std::remove_const_t<Value>>, field_offsetor(data),
              ConstFlag{}};
    } else {
      return {TypeID::of<std::remove_const_t<Value>>,
    ...
    tregistry.Register<Value>();
    RegisterTypeAuto<std::remove_const_t<Value>>();
    return {TypeID::of<std::remove_const_t<Value>>, data,
    ...
    return {TypeID::of<std::remove_const_t<RawT>>, buffer};
  } else {
    ...
    return {TypeID::of<std::remove_const_t<Value>>, offsetor, ConstFlag{}};
  }
}
...
ResultDesc ReflMngr::GenerateResultDesc() {
  if constexpr (!std::is_void_v<Return>) {
    ...
    return {TypeID::of<Return>, sizeof(U), alignof(U)};
  } else
    return {};
}
...
void ReflMngr::RegisterTypeAuto(AttrSet attrs_ctor, AttrSet attrs_dtor) {
  ...
  if constexpr (std::is_void_v<T>)
    ...
    if constexpr (std::is_const_v<T>)
      RegisterTypeAuto<std::remove_const_t<T>>(std::move(attrs_ctor),
                                               std::move(attrs_dtor));
    else if constexpr (std::is_reference_v<T>)
      RegisterTypeAuto<std::remove_reference_t<T>>(std::move(attrs_ctor),
                                                   std::move(attrs_dtor));
    else if constexpr (std::is_pointer_v<T>)
      RegisterTypeAuto<std::remove_pointer_t<T>>(std::move(attrs_ctor),
                                                 std::move(attrs_dtor));
    else {
...
StrID ReflMngr::AddField(std::string_view name, AttrSet attrs) {
  ...
  if constexpr (std::is_enum_v<FieldData>) {
    return AddField(TypeID::of<std::remove_const_t<FieldData>>, name,
                    {GenerateFieldPtr<field_data>(), std::move(attrs)});
  } else if constexpr (std::is_pointer_v<FieldData>) {
    return AddField(
        TypeID::of<std::remove_const_t<std::remove_pointer_t<FieldData>>>, name,
        {GenerateFieldPtr<field_data>(), std::move(attrs)});
  } else {
    return AddField(
        TypeID::of<
            std::remove_const_t<member_pointer_traits_object<FieldData>>>,
...
StrID ReflMngr::AddField(std::string_view name, T&& data, AttrSet attrs) {
  ...
  if constexpr (std::is_member_object_pointer_v<RawT>)
    return AddField(TypeID::of<member_pointer_traits_object<RawT>>, name,
                    std::forward<T>(data), std::move(attrs));
  else if constexpr (std::is_enum_v<RawT>)
    return AddField(TypeID::of<RawT>, name, std::forward<T>(data),
                    std::move(attrs));
  else {
    using Traits = FuncTraits<RawT>;
    ...
    return AddField(TypeID::of<Obj>, name, std::forward<T>(data),
                    std::move(attrs));
  }
}
...
T ReflMngr::InvokeRet(TypeID typeID, StrID methodID,
                      Span<const TypeID> argTypeIDs, void* args_buffer) const {
  using U =
...
SharedObject ReflMngr::MInvoke(TypeID typeID, StrID methodID,
  ...
  if constexpr (sizeof...(Args) > 0) {
    ...
    return MInvoke(typeID, methodID, Span<const TypeID>{},
                   static_cast<void*>(nullptr), rst_rsrc);
}
...
template <typename... Args>
SharedObject ReflMngr::DMInvoke(TypeID typeID, StrID methodID, Args... args) {
  return MInvoke<Args...>(typeID, methodID, std::pmr::get_default_resource(),
...
SharedObject ReflMngr::DMInvoke(ConstObjectPtr obj, StrID methodID,
                                Args... args) {
  return MInvoke<Args...>(obj, methodID, std::pmr::get_default_resource(),
                          std::forward<Args>(args)...);
}

template <typename... Args>
SharedObject ReflMngr::DMInvoke(ObjectPtr obj, StrID methodID, Args... args) {
  return MInvoke<Args...>(obj, methodID, std::pmr::get_default_resource(),
                          std::forward<Args>(args)...);
}
}  // namespace My::MyDRefl
