...
struct TypeAutoRegister_Default {
  static void run(ReflMngr& mngr) {
    ...
    if constexpr (IsTuple<T> && !IsArray<T>) {
      ...
      mngr.AddMemberMethod(
          NameIDRegistry::Meta::tuple_get,
          [](T& t, const std::size_t& i) { return runtime_get(t, i); });
      mngr.AddMemberMethod(
          NameIDRegistry::Meta::tuple_get,
          [](const T& t, const std::size_t& i) { return runtime_get(t, i); });
      mngr.AddMemberMethod(
          NameIDRegistry::Meta::tuple_get,
          [](T& t, const Type& type) { return runtime_get(t, type); });
      mngr.AddMemberMethod(
          NameIDRegistry::Meta::tuple_get,
          [](const T& t, const Type& type) { return runtime_get(t, type); });
      mngr.AddMemberMethod(NameIDRegistry::Meta::tuple_element,
                           [](const T& t, const std::size_t& i) {
                             return runtime_tuple_element(t, i);
                           });
    ...
    if constexpr (container_remove<T>)
      mngr.AddMemberMethod(
          NameIDRegistry::Meta::container_remove,
          [](T& lhs, const typename T::value_type& v) -> std::size_t {
    ...
    if constexpr (container_unique<T>)
      mngr.AddMemberMethod(NameIDRegistry::Meta::container_unique,
                           [](T& lhs) -> std::size_t {
