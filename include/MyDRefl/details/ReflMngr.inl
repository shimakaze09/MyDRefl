...
ParamList ReflMngr::GenerateParamList(
    Span<NameID, sizeof...(Params)> paramNameIDs) const
    noexcept(sizeof...(Params) == 0) {
  if constexpr (sizeof...(Params) > 0) {
    std::vector<Parameter> params{
        {tregistry.GetID<Params>(), sizeof(Params), alignof(Params)}...};
...
T ReflMngr::Invoke(TypeID typeID, NameID methodID, Args... args) const {
  if constexpr (sizeof...(Args) > 0) {
    std::array argTypeIDs = {TypeRegistry::DirectGetID<Args>()...};
    auto args_buffer =
        remove_lref_as_tuple_buffer<Args...>(std::forward<Args>(args)...);
