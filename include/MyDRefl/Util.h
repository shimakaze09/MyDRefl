...
//     |       T[] |  1  |     0     |  -  |     0     |
//     | const T[] |  1  |     1     |  1  |     -     |
constexpr bool is_pointer_array_compatible(std::string_view to,
                                           std::string_view from) noexcept;
...
template <typename T, typename U>
concept operator_assign = requires(T lhs, U rhs) {
  {lhs = std::forward<U>(rhs)}->std::same_as<T&>;
};
template <typename T>
concept operator_assign_copy =
    std::is_copy_assignable_v<T> && operator_assign<T, const T&>;
template <typename T>
concept operator_assign_move =
    std::is_move_assignable_v<T> && operator_assign<T, T&&>;
template <typename T>
...
template <typename T>
concept type_ctor_move =
    std::is_move_constructible_v<T> && type_ctor<T, const T&>;
