...
concept operator_lshift = requires(T lhs, U rhs) {
  lhs << rhs;
};
template <typename T, typename U>
concept operator_rshift = requires(T lhs, U rhs) {
  lhs >> rhs;
};
...
template <typename T>
concept operator_assign_copy =
    std::is_copy_assignable_v<T> && requires(T lhs, const T& rhs) {
  {lhs = rhs}->std::same_as<T&>;
};
template <typename T>
concept operator_assign_move =
    std::is_move_assignable_v<T> && requires(T lhs, T rhs) {
  {lhs = std::move(rhs)}->std::same_as<T&>;
};
template <typename T>
concept operator_assign_add =
    !std::is_same_v<T, bool> && requires(T lhs, const T& rhs) {
  {lhs += rhs}->std::same_as<T&>;
};
template <typename T>
concept operator_assign_sub =
    !std::is_same_v<T, bool> && requires(T lhs, const T& rhs) {
  {lhs -= rhs}->std::same_as<T&>;
};
template <typename T>
concept operator_assign_mul =
    !std::is_same_v<T, bool> && requires(T lhs, const T& rhs) {
  {lhs *= rhs}->std::same_as<T&>;
};
template <typename T>
concept operator_assign_div =
    !std::is_same_v<T, bool> && requires(T lhs, const T& rhs) {
  {lhs /= rhs}->std::same_as<T&>;
};
template <typename T>
concept operator_assign_mod =
    !std::is_same_v<T, bool> && requires(T lhs, const T& rhs) {
  {lhs %= rhs}->std::same_as<T&>;
};
template <typename T>
concept operator_assign_band =
    !std::is_same_v<T, bool> && requires(T lhs, const T& rhs) {
  {lhs &= rhs}->std::same_as<T&>;
};
template <typename T>
concept operator_assign_bor =
    !std::is_same_v<T, bool> && requires(T lhs, const T& rhs) {
  {lhs |= rhs}->std::same_as<T&>;
};
template <typename T>
concept operator_assign_bxor =
    !std::is_same_v<T, bool> && requires(T lhs, const T& rhs) {
  {lhs ^= rhs}->std::same_as<T&>;
};
template <typename T>
concept operator_assign_lshift =
    !std::is_same_v<T, bool> && requires(T lhs, const T& rhs) {
  {lhs <<= rhs}->std::same_as<T&>;
};
template <typename T>
concept operator_assign_rshift =
    !std::is_same_v<T, bool> && requires(T lhs, const T& rhs) {
  {lhs >>= rhs}->std::same_as<T&>;
};
...
template <typename T>
concept operator_deref = requires(T t) {
  *t;
};
template <typename T>
concept operator_ref = requires(T t) {
  &t;
};
...
concept container_ctor_cnt =
    container_size_type<T> && requires(const typename T::size_type& cnt) {
  T{cnt};
};

template <typename T>
concept container_ctor_clvalue =
    container_value_type<T> && requires(const typename T::value_type& v) {
  T{v};
};

template <typename T>
concept container_ctor_rvalue =
    container_value_type<T> && requires(typename T::value_type && v) {
  T{std::move(v)};
};
...
concept container_ctor_cnt_value =
    ...
    requires(const typename T::size_type& cnt,
...
concept container_ctor_ptr_cnt =
    ...
    requires(const typename T::pointer_type& first,
...
concept container_ctor_ptr_ptr =
    ...
    requires(const typename T::pointer_type& first,
             const typename T::pointer_type& last) {
  T{first, last};
};
...
concept container_subscript_key_cl =
    ...
    container_subscript<T, const typename T::key_type&>;
template <typename T>
...
concept container_empty = requires(const T& t) {
  {std::empty(t)}->std::convertible_to<bool>;
};
template <typename T>
...
concept container_insert_clvalue =
    ...
    container_insert<T, const typename T::value_type&>;
template <typename T>
concept container_insert_rvalue =
    container_value_type<T> && container_insert<T, typename T::value_type&&>;
template <typename T>
concept container_insert_rnode =
    container_node_type<T> && container_insert<T, typename T::node_type&&>;
...
concept container_insert_citer_clvalue =
    container_value_type<T> &&
    container_insert_citer<T, const typename T::value_type&>;
template <typename T>
concept container_insert_citer_rvalue =
    container_value_type<T> &&
    container_insert_citer<T, typename T::value_type&&>;
template <typename T>
concept container_insert_citer_rnode =
...
concept container_insert_or_assign = requires(T t, U u, V v) {
  t.insert_or_assign(std::forward<U>(u), std::forward<V>(v));
};
template <typename T>
concept container_insert_or_assign_clkey_rmap =
    container_key_type<T> && container_mapped_type<T> &&
    container_insert_or_assign<T, const typename T::key_type&,
                               typename T::mapped_type&&>;
template <typename T>
concept container_insert_or_assign_rkey_rmap =
    container_key_type<T> && container_mapped_type<T> &&
    container_insert_or_assign<T, typename T::key_type&&,
                               typename T::mapped_type&&>;
...
concept container_insert_or_assign_citer_clkey_rmap =
    container_key_type<T> && container_mapped_type<T> &&
    container_insert_or_assign_citer<T, const typename T::key_type&,
                                     typename T::mapped_type&&>;
template <typename T>
concept container_insert_or_assign_citer_rkey_rmap =
    container_key_type<T> && container_mapped_type<T> &&
...
concept container_push_front_clvalue =
    container_value_type<T> &&
    container_push_front<T, const typename T::value_type&>;
template <typename T>
concept container_push_front_rvalue =
    container_value_type<T> &&
...
concept container_push_back_clvalue =
    container_value_type<T> &&
    container_push_back<T, const typename T::value_type&>;
template <typename T>
concept container_push_back_rvalue =
...
concept container_extract_citer =
    container_const_iterator<T> &&
    container_extract<T, typename T::const_iterator>;
template <typename T>
concept container_extract_key =
    container_key_type<T> && container_extract<T, typename T::key_type>;
...
concept container_count = container_key_type<T> &&
  ...
  {t.count(u)}->std::convertible_to<std::size_t>;
};
...
concept container_contains =
    container_key_type<T> &&
    requires(const T& t, const typename T::key_type& u) {
  {t.count(u)}->std::convertible_to<bool>;
};
