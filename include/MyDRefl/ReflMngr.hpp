...
class MyDRefl_core_API ReflMngr {
 public:
  ...
  bool AddField(Type type, Name name, T&& data, AttrSet attrs = {}) {
    return AddField(
        type, name,
        {GenerateFieldPtr<T, NeedRegisterFieldType>(std::forward<T>(data)),
         std::move(attrs)});
  }
  ...
  bool SimpleAddField(Type type, Name name, T&& data, AttrSet attrs = {}) {
    return AddField<T, false>(type, name, std::forward<T>(data),
                              std::move(attrs));
  }
  ...
  template <typename T, typename... Args>
  bool SimpleAddDynamicFieldWithAttr(Type type, Name name, AttrSet attrs,
                                     Args&&... args) {
    return AddField(
        type, name,
        {GenerateDynamicFieldPtr<T, false>(std::forward<Args>(args)...),
         std::move(attrs)});
  }
  ...
  bool SimpleAddDynamicField(Type type, Name name, Args&&... args) {
    return SimpleAddDynamicFieldWithAttr<T>(type, name, {},
                                            std::forward<Args>(args)...);
  }
  ...
  //     |       T[] |  1  |     0     | -/1 |     0     |
  //     | const T[] |  1  |     1     |  1  |    -/1    |
  bool IsCompatible(std::span<const Type> paramTypes,
