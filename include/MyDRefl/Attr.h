...
template<>
struct std::hash<My::MyDRefl::Attr> {
    std::size_t operator()(const My::MyDRefl::Attr& obj) noexcept {
        return std::hash<const void*>()(obj.get());
    }
};

inline bool operator==(const My::MyDRefl::Attr& left, const My::MyDRefl::Attr& right) noexcept {
...
std::basic_ostream<Elem, Traits>& operator<<(std::basic_ostream<Elem, Traits>& out, const My::MyDRefl::Attr& obj) {
    return out << obj.get();
}

namespace std {
    inline void swap(My::MyDRefl::Attr& left, My::MyDRefl::Attr& right) noexcept {
        left.swap(right);
    }

    inline void swap(My::MyDRefl::Attr& left, shared_ptr<void>& right) noexcept {
        left.swap(right);
    }

    inline void swap(shared_ptr<void>& left, My::MyDRefl::Attr& right) noexcept {
        right.swap(left);
    }
}
