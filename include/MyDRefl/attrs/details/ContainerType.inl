...
concept IsVector =
    ...
    container_insert_citer_rvalue<T> && container_insert_citer_cnt<T> &&
...
concept IsDeque =
    ...
    container_insert_citer_cnt<T> && container_erase_citer<T> &&
...
concept IsForwardList =
    ...
    && container_front<T> && container_front<const T> && container_empty<T> &&
    container_resize_cnt<T> && container_resize_cnt_value<T>

    && container_clear<T> && container_insert_after_clvalue<T> &&
    container_insert_after_rvalue<T> && container_insert_after_cnt<T> &&
    container_erase_after<T> && container_erase_after_range<T> &&
    container_push_front_clvalue<T> && container_push_front_rvalue<T> &&
    container_pop_front<T> && container_swap<T> && container_merge_l<T> &&
    container_merge_r<T>

    && container_splice_after_l<T> && container_splice_after_r<T> &&
    container_splice_after_it_l<T> && container_splice_after_it_r<T> &&
    container_splice_after_range_l<T> && container_splice_after_range_r<T> &&
    container_remove<T> && container_reverse<T> && container_unique<T> &&
    container_sort<T>;
...
concept IsList =
    ...
    && container_front<T> && container_front<const T> && container_back<T> &&
    container_back<const T> && container_empty<T> && container_size<T> &&
    container_resize_cnt<T> && container_resize_cnt_value<T>
    ...
    container_erase_citer<T> && container_erase_range_citer<T> &&
    container_push_front_clvalue<T> && container_push_front_rvalue<T> &&
    container_pop_front<T> && container_push_back_clvalue<T> &&
    container_push_back_rvalue<T> && container_pop_back<T> &&
    container_swap<T> && container_merge_l<T> && container_merge_r<T>

    && container_splice_l<T> && container_splice_r<T> &&
    container_splice_it_l<T> && container_splice_it_r<T> &&
    container_splice_range_l<T> && container_splice_range_r<T> &&
    container_remove<T> && container_reverse<T> && container_unique<T> &&
    container_sort<T>;
...
template <typename T>
concept IsStack = true && container_top<T> && container_top<const T> &&
                  container_empty<T> && container_size<T>

                  && container_push_clvalue<T> && container_push_rvalue<T> &&
...
concept IsQueue = true && container_front<T> && container_front<const T> &&
                  ...
                  && container_pop<T> && container_push_clvalue<T> &&
                  container_push_rvalue<T> && container_swap<T>;
...
template <typename T>
concept IsOptional =
    true && container_value_type<T> && optional_has_value<T> &&
    optional_value<T> && optional_value<const T> && optional_reset<T>;
